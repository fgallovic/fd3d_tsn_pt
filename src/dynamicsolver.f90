    ...
    if(RUNI==0.or.RUNI==10)then !    Call the forward modelling only
      ...
      elseif(iwaveform==4.or.iwaveform==5.or.iwaveform==45)then
        ...
        write(594)-1.,-1.,T0I(:,:),TsI(:,:),DcI(:,:),ruptime(nabc+1:nxt-nabc,nabc+1:nzt-nfs),slipZ(nabc+1:nxt-nabc,nabc+1:nzt-nfs), &
          & rise(nabc+1:nxt-nabc,nabc+1:nzt-nfs),schangeZ(nabc+1:nxt-nabc,nabc+1:nzt-nfs),peaksliprate(nabc+1:nxt-nabc,nabc+1:nzt-nfs),efracds(nabc+1:nxt-nabc,nabc+1:nzt-nfs),MomentRate(:),M0,Eg,Er,-1000.,output_param(1),output_param(4),-1.,output_param(3)
#else
        write(594)-1.,-1.,T0I(:,:),TsI(:,:),DcI(:,:),ruptime(nabc+1:nxt-nabc,nabc+1:nzt-nfs),slipX(nabc+1:nxt-nabc,nabc+1:nzt-nfs), &
          & rise(nabc+1:nxt-nabc,nabc+1:nzt-nfs),schangeX(nabc+1:nxt-nabc,nabc+1:nzt-nfs),peaksliprate(nabc+1:nxt-nabc,nabc+1:nzt-nfs),efracds(nabc+1:nxt-nabc,nabc+1:nzt-nfs),MomentRate(:),M0,Eg,Er,-1000.,output_param(1),output_param(4),-1.,output_param(3)
#endif
        close(594)
      endif
