    ...
    REAL*8, PARAMETER:: maxampl=.9d0   !maximum amplitude of the slip vel funcs
...
!    open(102,FILE='inputtf.dat')
    ...
    open(293,FILE='mtildeslip2D.dat')
    do kk=1,NSeg
...
!            read(102,*)(CM(i,j,k),m=1,NM)
      ...
      do k=1,NW(kk)
        ...
        enddo
        write(293,'(1000E13.5)')(sum(mtilde(:,j,k))*dt,j=1,NL(kk)),sum(mtilde(:,NL(kk),k))*dt
      enddo
      write(293,'(1000E13.5)')(sum(mtilde(:,j,NW(kk)))*dt,j=1,NL(kk)),sum(mtilde(:,NL(kk),NW(kk)))*dt
      write(292,*);write(292,*)
